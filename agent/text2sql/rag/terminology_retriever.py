...
async def _select_terminology_by_word(
    ...
    if datasource_id is not None:
        ...
        stmt = stmt.filter(
            or_(
                or_(Terminology.specific_ds == 0, Terminology.specific_ds.is_(None)),
                and_(
                    Terminology.specific_ds == 1,
        ...
        stmt = stmt.filter(
            or_(Terminology.specific_ds == 1, Terminology.specific_ds.is_(None))
        )
    ...
    for term in keyword_results:
        if term.parent_id is not None:
            terminology_ids.add(term.parent_id)
        else:
            ...
            if embedding:
                ...
                if datasource_id is not None:
                    embedding_sql = text(f"""
                        SELECT id, parent_id, word, similarity
                        FROM (
                            SELECT id, parent_id, word,  specific_ds, datasource_ids, enabled_flag,
                        ...
                        WHERE similarity > {EMBEDDING_TERMINOLOGY_SIMILARITY} 
                          AND enabled_flag = 1
                          AND (
                              (specific_ds = 0 OR specific_ds IS NULL)
                              OR
                              (specific_ds = 0 AND datasource_ids IS NOT NULL 
                    ...
                    embedding_sql = text(f"""
                        ...
                        WHERE similarity > {EMBEDDING_TERMINOLOGY_SIMILARITY} 
                          AND enabled_flag = 1
                          AND (specific_ds = 0 OR specific_ds IS NULL)
                        ORDER BY similarity DESC
                        LIMIT {EMBEDDING_TERMINOLOGY_TOP_COUNT}
                    """)
                    params = {
                ...
                for row in embedding_results:
                    if row.parent_id is not None:
                        terminology_ids.add(row.parent_id)
                    else:
    ...
    all_terms = session.query(Terminology).filter(
        or_(Terminology.id.in_(list(terminology_ids)), Terminology.parent_id.in_(list(terminology_ids)))
    ).all()

    # 组织为字典格式
    term_dict = {}
    for term in all_terms:
        pid = term.parent_id if term.parent_id is not None else term.id
