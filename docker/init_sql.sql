...
-- t_api_token definition
DROP TABLE IF EXISTS t_api_token CASCADE;
...
CREATE UNIQUE INDEX uk_t_api_token_hash ON t_api_token(token_hash);
CREATE INDEX idx_t_api_token_user_id ON t_api_token(user_id);
CREATE INDEX idx_t_api_token_status ON t_api_token(status);
